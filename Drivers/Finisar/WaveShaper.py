...
def parse_wsp(wsp_text, unwrap=True):
    ...
    '''
    wsp_array = np.array([np.array([float(item) for item in row.split('\t')]) for row in wsp_text.split('\n')]).T
    if (unwrap==True):
...
class WS1000():
    def __init__(self,config_file, name='ws1'):
        '''Initialize a WaveShaper object.
    ...
    def filter_profile(self, set_profile=None, return_profile=True):
        ...
        # Read profile from WaveShaper
            self.buffer_size = ws_get_profile(self.name, psize=1, return_psize=True)[1]
            wsp_profile = ws_get_profile(self.name, psize=self.buffer_size)
            (self.freq, self.attn, self.phase, self.port) = parse_wsp(wsp_profile)
            if (return_profile==True):
                return (self.freq, self.attn, self.phase, self.port)
        elif isinstance(set_profile,dict):
        # Write predefined profile to WaveShaper
            ws_load_predefinedprofile(self.name, **set_profile)
            self.filter_profile(return_profile=False)
        else:
        # Write custom WSP profile
            ws_load_profile(self.name, format_wsp(*set_profile))
            self.filter_profile(return_profile=False)
    ...
    def phase_profile(self, set_profile=None):
        ...
        # Write custom phase profile
            self.filter_profile(set_profile=(self.freq, self.attn, set_profile, self.port), return_profile=False)
    ...
    def attn_profile(self, set_profile=None):
        ...
        # Write custom attenuation profile
            self.filter_profile(set_profile=(self.freq, set_profile, self.phase, self.port), return_profile=False)
...
def ws_load_predefinedprofile(name, filtertype=PROFILE_TYPE_TRANSMIT, center=0.0, bandwidth=0.0, attn=0.0, port=0):
...
def ws_get_profile(name, psize=512*1024, return_psize=False):
