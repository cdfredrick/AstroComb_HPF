...
class StageOne(object):
    ...
    def __init__(self, rio_card=RIO_CARD_NUM, preamp_card=PREAMP_CARD_NUM,
        ...
        self.rio_pd_monitor = simple_daq_driver.SimpleDAQ(pd_chan, pd_threshold)
    ...
    def enable_rio(self):
        ...
        if not self.rio_laser.query_tec_on():
            ...
            if not self.rio_laser.query_tec_on():
                log.log_warn(__name__, 'enable_rio', 'Rio TEC will not turn on!')
                return False
        self.rio_laser.enable_las(True)
        if not self.rio_laser.query_las_on():
            log.log_warn(__name__, 'enable_rio', 'Rio laser will not turn on!')
            return False
        return True
    ...
    def enable_preamp(self):
        ...
        if not self.preamp.query_tec_on():
            ...
            if not self.preamp.query_tec_on():
                log.log_warn(__name__, 'enable_preamp', 'Preamp TEC will not turn on!')
                return False
        self.preamp.enable_las(True)
        if not self.preamp.query_las_on():
            log.log_warn(__name__, 'enable_rio', 'Preamp laser will not turn on!')
            return False
        return True
