...
dev = {}
    # DAQ drivers
dev['comb_generator/device_PDU_12V'] = {
        'driver':send_args(PDUOutlet, DEVICE_SETTINGS['comb_generator/device_PDU_12V']['__init__']),
        'queue':CouchbaseDB.PriorityQueue('192.168.0.2')}
...
def outlet_on(state_db):
    ...
    # Outlet is off
        raise Exception('Outlet is off, manually enable before starting this script.')

# Maintain Functions ----------------------------------------------------------
'''This section is for defining the methods needed to maintain the system in
    its defined states.'''
max_temperature_threshold = 0.35
def below_temp_threshold(state_db):
    device_db = 'comb_generator/device_PDU_12V'
    # Queue
    dev[device_db]['queue'].queue_and_wait()
    # Get outlet state
    ...
    # Check the box temperature
        if mon['ambience/box_temperature_0']['new']:
            if mon['ambience/box_temperature_0']['data'][-1] > max_temperature_threshold:
            # Shut it down if the temperature is too high
        ...
        'comb_generator/state_12V_supply':{
                'read':{
                        ...
                        'prerequisites':{
                                ...
                                'optional':[]},
                        'routines':{
                                'monitor':monitor, 'search':outlet_on,
                                'maintain':below_temp_threshold, 'operate':nothing}},
                'safe':{
                        'settings':{},
                        'prerequisites':{
                                'critical':[],
