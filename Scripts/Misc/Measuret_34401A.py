...
temperature_record = open("temperature_record5.txt",'a')
time_then = time.time() - 60

while True:
    time_now = time.time()
    if time_now - time_then > 60:
    # Read Measurement ------------------------------------------------------------
        t = float(ctr.query("MEAS:RES?")) # Read resistance
        print("{:}\t{:.8g}\n".format(time_now, t))
        temperature_record.write("{:}\t{:.6g}\n".format(time_now, t))
        temperature_record.flush()
        time_then = time_now
